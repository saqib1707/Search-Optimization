    ...
    def loop(self):
        ...
        for i in range(max_itr):
            ...
            if self.auto and not game_over:
                ai = AI(self.game.get_state(), TREE_DEPTH)
                if not self.ec:
            ...
            if direction != None:
                ...
                curr_game_state = self.game.get_state()
                _, curr_game_score = curr_game_state
                game_score_lst.append(curr_game_score)
                # print("Move ", i+1, " Score: ", curr_game_score)
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        game = GameRunner()
        game_score_lst = game.loop()

        plt.plot(range(100), game_score_lst)
    ...
    #     plt.plot(range(100), game_score_lst, color='g')

    plt.title('Game Score | Exp-'+str(TREE_DEPTH))
    plt.xlabel('Move / Iteration')
    plt.ylabel('Game Score')
    plt.grid(linestyle = '--')
    plt.show()
    fig.savefig('../../plots/Q4_exp'+str(TREE_DEPTH)+'_score_plot.png')