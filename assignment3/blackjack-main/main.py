    ...
    def loop_for_plotting(self):
        ...
        for itr in range(5000):
            ...
            if self.autoPlay:
                ...
                # computing the cumulative win rate
                num_plays = self.game.winNum + self.game.loseNum
                if num_plays > 0:
                    num_plays_vec.append(num_plays)
                    win_rate_vec.append(self.game.winNum / num_plays)
        ...
        fig = plt.figure()
        plt.plot(num_plays_vec, win_rate_vec, "r")
        plt.title('Cumulative Win Rate VS Number of Plays')
        plt.xlabel('Number of Game Plays (N)')
        plt.ylabel('Win Rate (W / N)')
        plt.grid(linestyle = '--')
        plt.legend(["Action - HIT", "Action - STAND"])
        plt.show()
        # fig.savefig("plots/Q_win_rate_vs_num_plays.png")
...
if __name__ == '__main__':
    if args.test == 1:
        ...
        # for plotting the cumulative win rate vs number of plays, uncomment the below line
        # game.loop_for_plotting()